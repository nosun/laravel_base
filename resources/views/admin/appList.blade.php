    ...
    <div class="page-content">
        <div class="page-header" style="position: relative">
            <h1>
                ...
                <a class="btn btn-xs btn-success insertinput" data-toggle="modal" data-target="#edit-form" style="float: right" data-title="添加app" data-value='create'>
                    <i class="ace-icon fa fa-plus bigger-120"></i>添加app
                </a>
            </h1>
        </div>
        ...
        <div class="widget-body" id="searchformdiv">
            <div class="profile-user-info profile-user-info-striped" style="width: 100%;">
                <form class="form-search" name="searchform" method="get" action="/admin/app" >
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="widget-box " style="margin:0px">
                    ...
                    @foreach ($list as $row )
                        <tr>
                            <td id="app_id_{{$row->app_id}}">{{ $row->app_id }}</td>
                            <td id="app_name_{{$row->app_id}}">{{ $row->app_name }}</td>
                            <td id="app_info_{{$row->app_id}}">{{ $row->app_info }}</td>
                            <input id="iphone_url_{{$row->app_id}}" value="{{$row->iphone_url}}" type="hidden">
                            {{--<td id="show_time_{{$row->app_id}}">{{$row->show_time}}</td>--}}
                            <td id="publish_time_{{$row->app_id}}"><?php if($row->publish_time){echo date('Y-m-d',$row->publish_time); }else{echo '';} ?></td>
                            <td>
                                <div class="hidden-sm hidden-xs btn-group">
                                    <button type="button" class="btn btn-xs btn-danger dangerinput" data-toggle="modal"
                                            data-target=".bs-example-modal-sm" title="删除" data-id="{{ $row->app_id }}">
                                        <i class="ace-icon fa fa-trash-o bigger-120"></i>
                                    </button>
                                    <a type="button" class="btn btn-xs btn-warning"  title="查看"
                                       href="/admin/app/{{$row->app_id}}">
                                        ...
                                        <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
                                            ...
                                            <li>
                                                <a class="tooltip-danger dangerinput" data-toggle="modal"
                                                   data-target=".bs-example-modal-sm" title="delete" data-id="{{ $row->app_id }}">
                                                <span class="red">
                        ...
                        <div class="row">
                            <div class="form-group">
                                ...
                                <div class="col-xs-12 col-sm-6">
                                    <input type="text" class="form-control" name="app_name" id="editDescription">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="description" class="col-sm-2 control-label">app_info</label>
                                <div class="col-xs-12 col-sm-6">
                                    <input type="text" class="form-control" name="app_info" id="editDescription">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="description" class="col-sm-2 control-label">苹果商店url</label>
                                <div class="col-xs-12 col-sm-6">
                                    <input type="text" class="form-control" name="iphone_url" id="editDescription">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="description" class="col-sm-2 control-label">发布时间</label>
                                <div class="col-xs-12 col-sm-6">
                                    <input class="form-control" id="editDescription" name="publish_time" type="text" data-date-format="yyyy-mm-dd" />
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        ...
        $('#edit-form').on('show.bs.modal', function (event) {
            ...
            if (value == 'edit'){
                ...
                var url     = '/admin/app/'+ id;
                var methord = 'put';
                modal.find("input[name='app_name']").val($('#app_name_'+id).html());
                modal.find("input[name='iphone_url']").val($('#iphone_url_'+id).val());
                modal.find("input[name='app_info']").val($('#app_info_'+id).html());
                modal.find("input[name='publish_time']").val($('#publish_time_'+id).html());
            }
        ...
        $(document).ready(function() {
            $('#myform').submit(function(event) {
                ...
                var formData = {
                    ...
                    'app_info':$('input[name=app_info]').val(),
                    'iphone_url':$('input[name=iphone_url]').val(),
